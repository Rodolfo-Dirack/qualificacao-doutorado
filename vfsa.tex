...
A Equação \ref{eq:3.10} é chamada equação do CRS não-hiperbólico, pois não deriva de uma expansão em série de Taylor
...
O Semblance entre a superfície de tempo de trânsito da Equação \ref{eq:3.9} e os dados adiquiridos é utilizado no critério de
convergência do algoritmo do VFSA Equação \ref{eq:3.1} em cada iteração. Os parâmetros $R_N$, $R_{NIP}$ e $\beta_0$ que melhor
ajustam a superfície da Equação \ref{eq:3.10} serão os parâmetros ótimos.